...
public class ViewController implements ActionListener, MouseListener{
	...
	public void search(String term){
		view.getDetailsPanel().setVisible(false);
	...
	public void placeGeoElementOnScreen(GeoEvent geo){
		...
		view.getDateArea().setText(StringUtilities.getStringFromXSD(geo.getDate()));
	...
	public void updateRecomendations() {
		...
		if(eventList.size()==0){
			JOptionPane.showMessageDialog(view, "No Recommendations found, you need to attend concerts.");
		}
		else{
	...
	public void getAttended(){
		...
		for(String s : db.getEventsAttended()){
			view.getResults().addElement(db.getModelInfoFromEvent(s));	
		}
		view.repaint();	
		view.validate();
	}
	...
	public void attend (String eventURI){
		...
		if(db.getEventsAttended().contains(eventURI)){
			JOptionPane.showMessageDialog(view, "Denne konserten er allerede attendet");
		}
		else{
			db.attend(eventURI);
		}
